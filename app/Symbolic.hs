...
{-# LANGUAGE DerivingStrategies, DerivingVia, GADTs, LambdaCase     #-}
{-# LANGUAGE MultiParamTypeClasses, PatternGuards, RankNTypes       #-}
{-# LANGUAGE ScopedTypeVariables, TemplateHaskell, TypeApplications #-}
{-# OPTIONS_GHC -Wall #-}
module Symbolic
  ( Symbolic(..), (.:*), (.:+), (.:-), (.:/), (.:**), x, y, Reducer,
    normalise
  ) where
...
import           Control.Lens                  (Plated (..), Prism', Rewrapped,
                                                Wrapped (Unwrapped), ala,
                                                makePrisms, review, transformM,
                                                (^.), (^?), _Wrapped')
...
import           Numeric.Algebra               (Abelian, Additive, Division,
                                                Group, LeftModule, Monoidal,
                                                Multiplicative, Rig,
                                                RightModule, Ring, Semiring,
                                                Unital)
...
data Symbolic
  ...
  | Symbolic :** Symbolic
    ...
    deriving (Additive, LeftModule Natural, RightModule Natural,
              LeftModule Integer, RightModule Integer,
              Multiplicative, Rig,  Ring, Unital,
              Monoidal, Group, Abelian, Semiring)
...
instance Show Symbolic where
  ...
  showsPrec d (Negate sym) = showParen (d > 6)
    $ showString "- " . showsPrec 10 sym
  showsPrec d (Fun fun args) =
    showParen (d > 9)
    $ showString fun
    . foldl' (\l r -> l . showChar ' ' . r) id
...
simpleCommBinM
  ...
  -> (Double -> w)
  -> Symbolic -> Reducer Symbolic
simpleCommBinM p op e = do
  let (_, facs) = unfoldBin p e
      (coes0, rests) =
        partitionEithers
        $ map (\case{Lit c -> Left c; e0 -> Right e0})
          facs
